...
export function renderMmrBackgroundTaskResult(
  ...
  const model = stripProvider(subDetails.reportedModel ?? subDetails.model);
  ...
  if (renderStatus !== "running" && (subDetails.usage || model)) {
    container.addChild(new Spacer(1));
    container.addChild(
      new WorkerStatusLineComponent(details.agent, subDetails.usage, subDetails.contextWindow, model, theme),
    );
  }
