...
Shortcuts: `Ctrl+Shift+S` / `Alt+M` pick a mode, `Ctrl+Space` cycles `smart → smartGPT → rush → test → large → deep → open`. Subagent execution uses a separate profile via `--mmr-subagent <name>` (see [Subagent profiles](#subagent-profiles)).
...
- Keys: `all` plus any locked mode (`smart`, `smartGPT`, `rush`, `test`, `large`, `deep`). `open`, `free`, and unknown keys are ignored with a warning.
